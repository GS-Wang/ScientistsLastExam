...
def main():
    ...
    paths={'baseline':spec.initial_program_path,
           'replica_exchange':spec.task_dir/'verification/reference_tempering.py',
           'greedy_256':spec.task_dir/'verification/probe_greedy_multistart.py',
           'no_exchange':spec.task_dir/'verification/ablation_no_exchange.py',
           'no_quench':spec.task_dir/'verification/ablation_no_quench.py'}
    ...
    report={'schema_version':1,'scope':'fixed public-method qualification pilot; not model calibration or global evidence',
            ...
            'upstream_task_head':'30501b1ab634b82b8a45adec7cddbb47baaf67d8',
            'candidate_plan_sha256':{k:digest(v) for k,v in sources.items()},
            'planned_calls':10,'completed_calls':0,'model_calls':0,'scientific_admission':'not_assessed',
            'timeout_seconds':600,'runtime':{'python':sys.version,'numpy':numpy.__version__,'scipy':scipy.__version__,
    ...
    for name,source in sources.items():
        ...
        for repeat in (1,2):
            with tempfile.TemporaryDirectory(prefix='sle_f6_pilot_') as tmp:
                candidate=Path(tmp)/'solution.py';candidate.write_bytes(source)
                started=time.monotonic()
                metrics=evaluate_candidate(spec,candidate,timeout_s=600)
