"""Fail-closed black-box wrapper for F6SpinGlassGroundState."""
...
INVALID = -1e18
TASK_ID = "Physics/F6SpinGlassGroundState"
...
def main() -> int:
    ...
    try:
        ...
        if completed.returncode != 0:
            raise RuntimeError("sle eval exited %d" % completed.returncode)
        metrics.update(json.loads(completed.stdout))
        metrics.setdefault("raw_score", metrics.get("combined_score"))
    except Exception as exc:  # noqa: BLE001 - wrapper serializes failures instead of falling back
